...
| `engine.PipelineReaderTripwire` | 120s | PROD wedge alarm (Option B): one reader per hydrate pipeline, acquired at start while holding nothing; queueing past pool width K = max(READERS, LANES) is normal admission. The tripwire firing = leaked reader / never-released parked producer → loud PANIC → batch rpcError. NEVER a fallback — mid-flight acquires (and the fetchSerial fallback they needed) no longer exist |
...
| `GO_IVM_DELIVER_TIMEOUT_SEC` | 150s | PROD tripwire on a row-plane delivery parked against a FULL addon TSFN queue (ABI v4: nonblocking enqueue + Go-side cancellable retry — THE G13 wedge fix; the pre-v4 blocking deliver parked uncancellably in cgo holding rp.mu). Normal unparks: queue drains (JS loop recovers — 40-47s stalls observed and survivable), pull-gate cancel (TS timeout/abandon → `goivm_stream_cancel`, ≤120s), group teardown. The deadline fires only past ALL of those → `[GO-IVM][DELIVER-TIMEOUT]` incident marker + stream failure (hydrate: consumer-refusal unwind; advance: panic → -32000 → CG teardown). 150s > TS 120s so slow-successes are never preempted. `[GO-IVM][PERF-NAPI] deliver queue-full stalls/timeouts` per window; sustained stalls = JS event-loop starvation (the upstream cause). Pump control frames have NO deadline (dropping one orphans its RPC): they park until host teardown — off-worker, wedges nothing |
